...
export default function LinkStatusDashboard({articleStatusInfo}){
  ...
  const formattedData = articleStatusInfo.map((articleInfo, index, arr) => {
    return {
      ...
      style: articleInfo.status === 'complete' ? 'bg-emerald-400 border-emerald-400' : 'bg-rose-800 border-rose-800',
    };
  });
      ...
      {formattedData.map((article) => (
        <li key={article} className="my-3 relative">
          <div className={`border-2 p-3 rounded-md bg-indigo-900 ${article.status === "error" ? 'border-rose-500' : 'border-emerald-400'}`}>
            ...
            {article.status === "error" && <p>Message: <a href={`#error-${article.id}`}className="text-rose-500 underline">{article.message[0]}</a></p>}
            ...
            <div className="my-3">
              <span className="text-xs">URL: </span><a className="text-xs underline" href={article.link}>{article.link}</a>
            </div>
